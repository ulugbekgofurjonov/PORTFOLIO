import { ChevronDown, Star, Sparkles, Code2, Rocket } from "lucide-react";
import { PERSONAL_INFO, STATS } from "../../utils/constants";
import { scrollToSection } from "../../hooks/useScrollSpy";
import FadeIn from "../animations/FadeIn";
import { useState, useEffect, useRef } from "react";
import RadialGradientBackground from "../backgrounds/RadialGradientBackground";

const Hero = () => {
  const [displayedText, setDisplayedText] = useState("");
  const [isTypingComplete, setIsTypingComplete] = useState(false);
  const [cursorVisible, setCursorVisible] = useState(true);

  const fullText = PERSONAL_INFO.name;
  const subtitle = "FRONTEND DEVELOPER PORTFOLIO";

  // Typing animation with cursor blink
  useEffect(() => {
    let currentIndex = 0;
    const typingInterval = setInterval(() => {
      if (currentIndex <= fullText.length) {
        setDisplayedText(fullText.slice(0, currentIndex));
        currentIndex++;
      } else {
        setIsTypingComplete(true);
        clearInterval(typingInterval);
      }
    }, 120);

    return () => clearInterval(typingInterval);
  }, []);

  // Cursor blink effect
  useEffect(() => {
    if (isTypingComplete) {
      const cursorInterval = setInterval(() => {
        setCursorVisible((prev) => !prev);
      }, 500);
      return () => clearInterval(cursorInterval);
    }
  }, [isTypingComplete]);

  return (
    <section
      className="relative min-h-screen flex items-center justify-center overflow-hidden py-20 sm:py-24"
    >
      <RadialGradientBackground/>
      {/* Content container */}
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full text-center">
        {/* Premium Badge with glassmorphism - Blue theme */}
        <FadeIn delay={0}>
          <div className="inline-flex items-center gap-2 sm:gap-3 px-4 sm:px-6 py-2.5 sm:py-3.5 mb-8 sm:mb-12 backdrop-blur-xl bg-white/5 border border-white/10 rounded-full shadow-2xl hover:bg-blue-500/10 hover:border-blue-400/30 transition-all duration-500 group hover:scale-105">
            <div className="relative">
              <Star className="w-4 sm:w-5 h-4 sm:h-5 text-blue-400 fill-blue-400" />
              <Sparkles className="w-2.5 sm:w-3 h-2.5 sm:h-3 text-blue-400 absolute -top-1 -right-1" />
            </div>
            <span
              className="text-xs sm:text-sm text-white tracking-widest font-semibold uppercase bg-gradient-to-r from-white to-blue-400 bg-clip-text group-hover:text-transparent transition-all duration-500"
              style={{ fontFamily: "var(--font-display), sans-serif" }}
            >
              {PERSONAL_INFO.title}
            </span>
            <Code2 className="w-3.5 sm:w-4 h-3.5 sm:h-4 text-blue-400/70 group-hover:rotate-12 transition-transform duration-500" />
          </div>
        </FadeIn>

        {/* Main Heading with typing effect and cursor */}
        <FadeIn delay={100}>
          <div className="relative mb-6 sm:mb-8 px-2">
            <h1
              className="text-white leading-tight break-words relative z-10"
              style={{
                fontSize: "clamp(2rem, 6vw + 1rem, 5rem)",
                lineHeight: "1.15",
                fontWeight: "700",
                letterSpacing: "-0.03em",
                fontFamily: "var(--font-display), sans-serif",
                textShadow:
                  "0 0 40px rgba(59, 130, 246, 0.5), 0 0 80px rgba(59, 130, 246, 0.2)",
              }}
            >
              <span className="bg-gradient-to-r from-blue-400 via-blue-500 to-blue-600 bg-clip-text text-transparent inline-block">
                {displayedText}
                {!isTypingComplete && (
                  <span
                    className={`inline-block w-[2px] h-[1.2em] bg-blue-100 ml-1 align-middle ${cursorVisible ? "opacity-100" : "opacity-0"}`}
                    style={{ transition: "opacity 0.1s" }}
                  />
                )}
              </span>
            </h1>
            {/* Glowing underline */}
            <div
              className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-24 sm:w-32 h-1 rounded-full blur-sm"
              style={{
                background:
                  "linear-gradient(90deg, transparent, rgba(255, 255, 255, 0.8), transparent)",
              }}
            />
          </div>
        </FadeIn>

        {/* Subtitle with gradient - Blue theme */}
        <FadeIn delay={150}>
          <div className="relative mb-4 sm:mb-6 px-2">
            <p
              className={`font-bold mb-2 bg-gradient-to-r from-blue-100 via-white to-blue-100 bg-clip-text text-transparent transition-opacity duration-1000 ${isTypingComplete ? "opacity-100" : "opacity-0"}`}
              style={{
                fontFamily: "var(--font-display), sans-serif",
                fontSize: "clamp(0.875rem, 2vw + 0.5rem, 1.5rem)",
              }}
            >
              {subtitle}
            </p>
            <div className="flex items-center justify-center gap-2 text-blue-100/70">
              <div
                className="w-6 sm:w-8 h-[2px] rounded-full"
                style={{
                  background:
                    "linear-gradient(90deg, transparent, rgba(255, 255, 255, 0.8))",
                }}
              />
              <Rocket className="w-3.5 sm:w-4 h-3.5 sm:h-4 text-blue-100" />
              <div
                className="w-6 sm:w-8 h-[2px] rounded-full"
                style={{
                  background:
                    "linear-gradient(90deg, rgba(255, 255, 255, 0.8), transparent)",
                }}
              />
            </div>
          </div>
        </FadeIn>

        {/* Description with enhanced glassmorphism */}
        <FadeIn delay={200}>
          <div className="max-w-2xl mx-auto mb-8 sm:mb-12 px-2">
            <div
              className="backdrop-blur-xl bg-gradient-to-br from-white/5 to-white/10 border border-white/15 rounded-2xl p-5 sm:p-7 shadow-2xl hover:bg-white/15 hover:border-blue-200/30 transition-all duration-500 group hover:scale-[1.02]"
              style={{
                boxShadow:
                  "0 20px 40px rgba(0, 0, 0, 0.3), inset 0 1px 0 rgba(255, 255, 255, 0.1)",
              }}
            >
              <p
                className="text-sm sm:text-base lg:text-lg text-white/95 leading-relaxed group-hover:text-white transition-colors duration-300"
                style={{ fontFamily: "var(--font-body), sans-serif" }}
              >
                Men frontend dasturchiman. Zamonaviy veb-texnologiyalar orqali
                foydalanuvchilar uchun qulay va tezkor interfeyslar yarataman!
              </p>
            </div>
          </div>
        </FadeIn>

        {/* Premium CTA Button - Blue theme */}
        <FadeIn delay={300}>
          <button
            onClick={() => scrollToSection("contact")}
            className="group mb-12 sm:mb-20 inline-block relative"
          >
            {/* Outer glow effect */}
            <div
              className="absolute -inset-2 rounded-[24px] blur-xl opacity-60 group-hover:opacity-100 transition-opacity duration-500"
              style={{
                background:
                  "linear-gradient(90deg, rgba(255, 255, 255, 0.6), rgba(147, 197, 253, 0.8), rgba(255, 255, 255, 0.6))",
              }}
            />

            {/* Button content */}
            <div
              className="relative z-10 bg-gradient-to-r from-blue-50 to-sky-100 text-gray-900 rounded-[20px] px-8 sm:px-12 py-3.5 sm:py-5 text-base sm:text-lg font-bold border-2 border-white/80 hover:scale-105 active:scale-95 transition-all duration-300 shadow-2xl flex items-center gap-2 sm:gap-3 group"
              style={{
                fontFamily: "var(--font-body), sans-serif",
                boxShadow: "0 10px 30px rgba(255, 255, 255, 0.4)",
              }}
            >
              <span className="bg-gradient-to-r from-blue-600 to-sky-500 bg-clip-text text-transparent group-hover:from-blue-700 group-hover:to-sky-600 transition-all duration-300">
                Bog'lanish
              </span>
              <Sparkles className="w-4 sm:w-5 h-4 sm:h-5 text-blue-500 group-hover:rotate-180 transition-transform duration-500" />
            </div>

            {/* Inner glow on hover */}
            <div
              className="absolute inset-0 blur-2xl rounded-[20px] opacity-0 group-hover:opacity-50 transition-opacity duration-500 -z-10"
              style={{
                background:
                  "radial-gradient(circle at center, rgba(255, 255, 255, 0.5), transparent 70%)",
              }}
            />
          </button>
        </FadeIn>

        {/* Premium Stats with cards - Enhanced design */}
        <FadeIn delay={400}>
          <div className="grid grid-cols-3 gap-3 sm:gap-4 lg:gap-6 max-w-4xl mx-auto px-2">
            {STATS.map((stat, index) => (
              <div
                key={index}
                className="group relative backdrop-blur-xl bg-gradient-to-br from-white/5 to-white/10 border border-white/15 rounded-xl sm:rounded-2xl p-3 sm:p-4 lg:p-6 hover:bg-white/15 hover:border-blue-200/40 transition-all duration-500 hover:scale-105 hover:-translate-y-2"
                style={{
                  boxShadow: "0 10px 20px rgba(0, 0, 0, 0.2)",
                }}
              >
                {/* Card glow on hover */}
                <div
                  className="absolute -inset-[1px] rounded-xl sm:rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-sm -z-10"
                  style={{
                    background:
                      "linear-gradient(90deg, rgba(255, 255, 255, 0.2), rgba(147, 197, 253, 0.4), rgba(255, 255, 255, 0.2))",
                  }}
                />

                <div className="text-center relative z-10">
                  <div
                    className="font-black mb-1 sm:mb-2"
                    style={{
                      fontSize: "clamp(1.5rem, 4vw + 0.5rem, 2.5rem)",
                      fontFamily: "var(--font-body), sans-serif",
                      textShadow: "0 0 20px rgba(255, 255, 255, 0.5)",
                      background:
                        "linear-gradient(135deg, #ffffff, #dbeafe, #bfdbfe)",
                      WebkitBackgroundClip: "text",
                      backgroundClip: "text",
                      color: "transparent",
                    }}
                  >
                    {stat.value}
                  </div>
                  <p
                    className="text-white/90 font-semibold tracking-wider uppercase group-hover:text-blue-100 transition-colors duration-300"
                    style={{
                      fontSize: "clamp(10px, 2vw, 13px)",
                      fontFamily: "var(--font-display), sans-serif",
                    }}
                  >
                    {stat.label}
                  </p>
                </div>

                {/* Corner accents */}
                <div className="absolute top-2 right-2 w-1.5 sm:w-2 h-1.5 sm:h-2 bg-blue-100 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                <div className="absolute bottom-2 left-2 w-1.5 sm:w-2 h-1.5 sm:h-2 bg-sky-100 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-150" />
              </div>
            ))}
          </div>
        </FadeIn>
      </div>

      {/* CSS Animations */}
      <style jsx>{`
        .delay-150 {
          animation-delay: 150ms;
        }
      `}</style>
    </section>
  );
};

export default Hero;